...
namespace emitter
{
    json_fsm_t::json_fsm_t(io::stream_t * stream)
        : stack_()
        , stream_(stream)
    ...
    json_fsm_t::~json_fsm_t()
    {
        while (stack_.back() == SEQ_VAL || stack_.back() == MAP_KEY) {
            while (stack_.back() == SEQ_VAL) {
                transition<SEQ_VAL>(*this, event_t<END_SEQ>());
            }
            while (stack_.back() == MAP_KEY) {
                transition<MAP_KEY>(*this, event_t<END_MAP>());
            }
        }
    ...
    void json_fsm_t::change(state_type state)
    {
        stack_.back() = state;
    }

    void json_fsm_t::push(state_type state)
    {
    ...
    void json_fsm_t::out(double val)
    {
        if (is_container_empty_) {
            is_container_empty_ = false;
            if (top() == SEQ_VAL)
    ...
    void json_fsm_t::out(int64_t val)
    {
        if (is_container_empty_) {
            is_container_empty_ = false;
            if (top() == SEQ_VAL)
    ...
    void json_fsm_t::out(char const * val, size_t len)
    {
        if (is_container_empty_) {
            is_container_empty_ = false;
            if (top() == SEQ_VAL)
    ...
    state_type json_fsm_t::top() const
    {
        return stack_.back();
    }

    void json_fsm_t::error(event_type event) const
    {
        exception::error_event(event, POS_);
    }

    json_fsm_t::operator bool() const
    {
        return !stack_.empty();
    }
}
